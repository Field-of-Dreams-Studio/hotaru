    ...
    impl UpgradeManager {
        ...
        pub async fn initiate_upgrade(
            ...
            let handler = self
                ...
                .ok_or_else(|| {
                    format!(
                        "No upgrade handler for {:?} -> {:?}",
                        from_protocol, target_protocol
                    )
                })?;
            ...
            self.upgrades
                .write()
                .await
                .insert(connection_id, upgrade_ctx);
        ...
        pub async fn process_upgrade(
            ...
            let result = match upgrade_ctx.state {
                ...
                UpgradeState::SendingResponse => {
                    handler.await_confirmation(ctx, &upgrade_ctx).await
                }
                UpgradeState::WaitingClientConfirmation => {
                    handler.perform_handoff(ctx, &upgrade_ctx).await
                }
        ...
        /// Complete an upgrade
        pub async fn complete_upgrade(&self, connection_id: i128) -> Option<UpgradeContext> {
...
impl crate::context::HyperContext {
    ...
    pub fn current_protocol(&self) -> HttpProtocol {
        match self.version {
            crate::context::HttpVersion::Http1_0 | crate::context::HttpVersion::Http1_1 => {
                HttpProtocol::Http1
            }
