...
use akari::{
    Value,
    extensions::{Locals, LocalsClone, Params, ParamsClone},
};

use hotaru_core::{
    app::application::App,
    connection::{ConnectionStatus, ProtocolRole, RequestContext},
...
impl HyperContext {
    ...
    pub fn new_server(request: Request<Body>, app: Arc<App>) -> Self {
...
impl HyperContext {
    ...
    pub fn switch_to_ws(&mut self) {
        use crate::upgrade::{
            HttpProtocol, HttpUpgradeType, UpgradeContext, UpgradeMetadata, UpgradeState,
        };
        ...
        self.connection_status =
            ConnectionStatus::SwitchProtocol(std::any::TypeId::of::<WebSocketProtocol>());
    }
    ...
    pub fn switch_to_h2(&mut self) {
        ...
        self.connection_status =
            ConnectionStatus::SwitchProtocol(std::any::TypeId::of::<HyperHttp2>());
    }
...
pub fn switch_protocol_response(
    response: Response<Body>,
    target_protocol: std::any::TypeId,
) -> HyperContext {
    let mut ctx = HyperContext::new_client(
        Request::builder()
            .method("GET")
