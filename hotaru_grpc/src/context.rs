...
use hotaru_core::connection::{ProtocolRole, RequestContext};
...
impl GrpcContext {
    ...
    fn extract_metadata(headers: &HeaderMap) -> MetadataMap {
        ...
        for (name, value) in headers {
            ...
            if !name_str.starts_with(':') && name_str != "content-type" && name_str != "user-agent"
            {
                if let Ok(value_str) = value.to_str() {
                    if let Ok(metadata_value) = value_str.parse() {
                        ...
                        let key: &'static str =
                            Box::leak(name.as_str().to_string().into_boxed_str());
